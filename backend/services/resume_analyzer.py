...
def analyze_full_resume(
    ...
    # ── Previously-dead helpers — now called ──────────────────────
    # Generate prioritized improvement suggestions via recommendation_engine
    sections = {
        'experience': ' '.join(
            e.get('description', '') for e in parsed_resume.get('experience', []) if isinstance(e, dict)
        ),
        'education': ' '.join(
            (e.get('degree') or '') + ' ' + (e.get('institution') or '')
            for e in parsed_resume.get('education', []) if isinstance(e, dict)
        ),
        'skills': ' '.join(parsed_resume.get('skills', [])),
        'summary': parsed_resume.get('professional_summary', '') or '',
        'projects': ' '.join(
            p.get('description', '') for p in parsed_resume.get('projects', []) if isinstance(p, dict)
        ),
    }
    recs_result = generate_all_recommendations(
        skill_validation_results=skill_validation,
        grammar_results=grammar_results,
        location_results=location_results,
        ...
        resume_keywords=list(keywords),
    )
    formatted_recs = format_recommendations_for_api(recs_result)
    # Flatten into human-readable suggestion strings for the API
    suggestions = [
        f"{r['priority_icon']} [{r['priority_label']}] {r['title']}: {r['description']}"
        for r in formatted_recs
    ]

    # Generate critical issues list (was also never called)
    ...
    # Generate improvement tips (was also never called)
    ...
    return {
        ...
        "experience_months":       experience_months,
        # Previously-dead fields — now populated
        "suggestions":     suggestions + improvements,
        "critical_issues": critical_issues,
        "recommendations_detail": formatted_recs,  # full structured recs for future use
    }
