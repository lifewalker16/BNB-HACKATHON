...
def _detect_mime_type(file_data: bytes, filename: str) -> Optional[str]:
    """Detect MIME type safely using python-magic, file signatures, or filename extensions."""
    ...
    # 2. Inspect magic byte signatures
    ...
    # 3. Fallback to standard library mimetypes
...
def validate_file(file_data: bytes, filename: str) -> Tuple[bool, str, Optional[str]]:
    ...
    if not mime_type:
        return False, 'Could not determine the file type. Please upload a PDF or Word document (.docx, .doc).', None
